...
class Car(object):
    ...
    def decellerate(self, amount):
        if True:
            ...
            new_velocity = self.velocity + frictionVec

            if new_velocity.get_length() > speed:
                self.velocity = Vec2d(0, 0)
            else:
    ...
    def collides(self, otherPos, otherBounds):
        ...
        for vec in axis:
            ...
            leftmostA = minA <= minB
            overlap = False
            if leftmostA and maxA >= minB:
                overlap = True
            if not leftmostA and maxB >= minA:
                overlap = True
            if not overlap:
                return False
        return True
    ...
    def dirToName(self, dir):
        ...
        if dir == Vec2d(1, 0):
            return 'east'
        raise Exception(str(dir) + ' is not a recognized dir.')
